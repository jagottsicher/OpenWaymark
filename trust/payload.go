...
// ReasonSucceeded). ParsePayload enforces the shape each kind requires; the
...
type Payload struct {
	...
	EvidenceURL string     // KindEntity or KindConcluded, informational (OWM-6 §3).
...
// wirePayload is the JSON shape on the wire. Level and Successor are
// pointers so that "absent" and "explicitly the zero value" are
// distinguishable — OWM-6 §3 requires telling them apart in both cases:
// kind:"entity" needs level present, kind:"sensor" needs it absent;
// reason:"succeeded" needs successor present, reason:"discontinued" needs
// it absent.
...
// MUST rules of OWM-6 §3: kind is "entity", "sensor" or "concluded" and
// nothing else; kind:"entity" requires a valid level (0-6); kind:"sensor"
// must not carry one; kind:"concluded" requires a valid reason, and a
// successor exactly when reason is "succeeded", never when it is
// "discontinued". An unknown JSON field is also rejected, the same
// convention node.LoadConfig already uses — a mistyped field is a setting
